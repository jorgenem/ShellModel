...
void id_particle_transitions(int trans_type)
{
   ...
   double           clebsch, value = D_ZERO, *j_occ;
...
/********  extra  *************/
   j_occ = vector(sizeof(double),Model[0].numj_N); 
/*************************/
...
/************************  Old  ****************************************
         ...
         read_id_eigenvector(File_eigen_init, init_num,
                        Info_id[0].num[0],Info_id[0].num[1], Eigen_vec); 
****************************************************************************/
                  ...
                  int numj_orb, double *j_occ, double *eigenVec)
{
  ...
  if(fread((void *)j_occ,(size_t)sizeof(double), (size_t) h_eigenData.numj_occ,
  ...
  }
   {
     ...
     if(fread((void *) temp_mem, (size_t) sizeof(float),        /* read the vector */
	      (size_t) totDim, filePtr) != (size_t) totDim) {
       printf("\n\n Error in function read_new_id_eigenvector():");
       printf("\nWrong number of elements in file %s",filename );
       printf("\nvector_num = %d elements = %d\n",eigenVecNum, totDim);
       exit(1);
     }   /* end of if-test  */
...
} // End: function read_new_id_eigenvector()
